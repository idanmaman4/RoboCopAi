...
def build_pid_stream(events):
    ...
    return pid_cache.values()


def build_pid_and_handle_windows(events):
    handle_pid_cache = defaultdict(list)
    windows = []
    general_windows = [] 
    ...
    for e in events:
        ...
        for handle in extract_handles(e):
            ...
            if buf or handle > 0xf000000000000000: 
                buf.append(e)
    ...
    for (proc,handle),group_self in handle_pid_cache.items(): 
        if handle > 0xf000000000000000 and group_self: 
...
def base_severity_from_score(score: float) -> str:
    if score < HIGH_THR:
        ...
        return "MEDIUM"
    elif score < UNDER_LOW:
        return "LOW"
    return "FILTER"

def evaluate_and_report2(label: str, windows, X, model, scaler,window_mapping):
    """
    ...
    for i, (p, s) in enumerate(zip(preds, scores)):
        ...
        sev = base_severity_from_score(float(s))
